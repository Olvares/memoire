function [t,s] = myModSIS_7(s0, coef, drate, ta, t0, sig, I)
...
A = @(t,xs,y) g(xs) * y / k;%(xs+xi); % Attack
B = @(xi) rr * xi; % Retablishment
Dyz = @(t,xi,y) ((t-tyz)>=0) * d * xi * y / k;%(xs+xi); % Caterpillar to Adult
R = @(z) rh * z; %* (1 - z/k); % Reproduction
Dwy = @(t,w) ((t-twy)>=0) * ga * w; % Egg to Larvae

myfSIS = @(t,y) (t>=t0) * [B(y(2)) - A(t,y(1),y(4));
    A(t,y(1),y(4)) - B(y(2)) - m(t,y(2));
    -m(t,y(2));% * y(4);
    Dwy(t,y(6)) - Dyz(t,y(2),y(4)) - (uy + 0.0*y(4))*y(4);
    Dyz(t,y(2),y(4)) - uz*y(5) + sig;
